...
public class AccesoBdatos {
	...
	public List<Socio> consultaLocalidad(String localidad){
		...
		if(localidad.isEmpty()==false) { //Si localidad es distinto de vacio ejecuta lo de dentro
			try {
				...
				if (iter.hasNext()) { //Si hay siguiente devolverá la lista con todos los socios, podriamos haber hecho lista2.size()>0
					lista = lista2;;
				}else {
	...
	public int actualizarSocio(int id,String nombre, int estatura, int edad, String localidad) {
		...
		em.persist(socioBuscado); // no haria falta, por que el commit ya hace por debajo un persist. El persist solo es necesario para introducir objetos nuevos a la base de datos
